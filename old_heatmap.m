...
for eta = 0
    rng(10*eta);
    h1 = stochastic_heatmap(A, dt, max_u, 100, eta, 25);
    rng(10*eta);
    h2 = supervision_heatmap(A, dt, max_u, k1, k2, 100, eta, 25);
    plot_heatmap(h1, h2);
end
...
function plot_heatmap(h1, h2)
    ...
    figure;
    own = [winter; 1. 1. 1.];
    heatmap(h1c, 'ColorLimits',[0 3], 'Colormap', own, 'GridVisible','off');
    colorbar('off')
    ax = axes;
    hold(ax,'on')
    plot(ax, [-1 -0.058], [1.487 -2], 'r', 'linewidth', 2.);
    plot(ax, [0.058 1], [2 -1.485], 'r', 'linewidth', 2.);
    %plot(ax, [2, 2.5], [3 4], 'r', 'linewidth', 2.);
    ax.Color = 'none';
    xlim([-1 1]);
    ylim([-2 2]);
    ax.XTick = [];
